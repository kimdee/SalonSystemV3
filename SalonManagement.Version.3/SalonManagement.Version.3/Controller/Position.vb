...
Public Class Position
    ...
    Public Function EditPosition() As Boolean
        Try
            Dim bool As Boolean = False
            Dim sql As String = "UPDATE position SET positionName=@0,positionStandardPay=@1,positionOvertime=@2,positionBasicPay=@3 WHERE positionID=@4;"
            If IsConnected() = True Then
    ...
    Public Sub ViewPosition(gv As DataGridView, lbl As Label)
        ...
        End Try
    End Sub
    Public Sub SearchPosition(kw As String, gv As DataGridView, lbl As Label)
        Try
            Dim sql As String
            Dim i As Integer = 0
            ''and accountID <> " & FrmMain.UserID & "
            sql = "SELECT * FROM Position WHERE (positionName LIKE '" & kw & "%' or positionStandardPay LIKE '" & kw & "%' or" _
                  + " positionOvertime LIKE '" & kw & "%' or positionBasicPay LIKE '" & kw & "%') ORDER BY positionName ASC;"
            If IsConnected() = True Then
                Dim cmd = New MySqlCommand(sql, getServerConnection)
                Dim reader As MySqlDataReader = cmd.ExecuteReader()
                gv.Rows.Clear()
