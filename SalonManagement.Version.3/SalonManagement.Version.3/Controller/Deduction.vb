...
Public Class Deduction
    ...
    Public Sub ViewDeduction(gv As DataGridView, lbl As Label)
        ...
        End Try
    End Sub
    Public Sub SearchDeduction(kw As String, gv As DataGridView, lbl As Label)
        Try
            Dim sql As String
            Dim i As Integer = 0
            ''and accountID <> " & FrmMain.UserID & "
            sql = "SELECT * FROM deduction WHERE (deductionType LIKE '" & kw & "%' or deductionAmount LIKE '" & kw & "%') ORDER BY deductionType ASC;"
            If IsConnected() = True Then
                Dim cmd = New MySqlCommand(sql, getServerConnection)
                Dim reader As MySqlDataReader = cmd.ExecuteReader()
                gv.Rows.Clear()
        ...
        Try
            ...
            If IsConnected() = True Then
                ...
                While reader.Read()
                    i = i + 1
                    With gv
                        .Rows.Add(reader(0), cb, i, reader(1), reader(2))
                    End With
                End While
                reader.Close()
