...
Public Class Service
    ...
    Public Sub ViewService(gv As DataGridView, lbl As Label)
        ...
        End Try
    End Sub
    Public Sub SearchService(kw As String, gv As DataGridView, lbl As Label)
        Try
            Dim sql As String
            Dim i As Integer = 0
            ''and accountID <> " & FrmMain.UserID & "
            sql = "SELECT * FROM service WHERE (serviceName LIKE '" & kw & "%' or serviceDescription LIKE '" & kw & "%' or" _
                  + " servicePrice LIKE '" & kw & "%') ORDER BY serviceName ASC;"
            If IsConnected() = True Then
                Dim cmd = New MySqlCommand(sql, getServerConnection)
                Dim reader As MySqlDataReader = cmd.ExecuteReader()
                gv.Rows.Clear()
                ...
                If gv.RowCount > 0 Then
                    lbl.Hide()
                Else
                    lbl.Text = "Service does not exist"
                    lbl.Show()
                End If
                gv.ClearSelection()
            End If
