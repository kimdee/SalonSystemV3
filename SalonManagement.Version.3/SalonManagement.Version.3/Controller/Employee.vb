...
Public Class Employee
    ...
    Public Sub ViewEmployee(gv As DataGridView, lbl As Label)
        ...
        End Try
    End Sub
    Public Sub SearchEmployee(kw As String, gv As DataGridView, lbl As Label)
        Try
            Dim sql As String
            Dim i As Integer = 0
            ''and accountID <> " & FrmMain.UserID & "
            sql = "SELECT * FROM Employee WHERE (employeeLN LIKE '" & kw & "%' or employeeFN LIKE '" & kw & "%' or" _
                  + " employeeMN LIKE '" & kw & "%' or employeeGender LIKE '" & kw & "%' or EmployeePosition LIKE '" & kw & "%') ORDER BY employeeLN,employeeFN,employeeMN ASC;"
            If IsConnected() = True Then
                Dim cmd = New MySqlCommand(sql, getServerConnection)
                Dim reader As MySqlDataReader = cmd.ExecuteReader()
                gv.Rows.Clear()
